...
class SpiderService:
    ...
    def __init__(self):
        self.spider = GDUTSpider(use_mock_fallback=True)
    
    def crawl_notices(self, max_pages=3) -> Dict:
        """
        爬取通知数据
        """
        try:
            success = self.spider.run(max_pages=max_pages)
            if success:
                notices = self.spider.get_notices()
                return {
                    'status': 'success',
                    'message': f'爬取成功，共获取 {len(notices)} 条通知',
                    'data': notices
                }
            else:
                return {
